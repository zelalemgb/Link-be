-- Migration: Update register_patient_with_visit to accept consultation_service_id
-- Description: Allows specifying the exact consultation service to charge during registration.

-- Drop the old function (with previous signature) to avoid ambiguity if we were reusing names, 
-- but since we are replacing, we just need to be careful with the signature.
-- The previous signature had 25 params. We are adding one more.

-- First, let's create the new function with the added parameter.
-- We purposely put it at the end (or near relevant params) to minimize disruption, 
-- though named parameters are safer.
-- We will add p_consultation_service_id UUID DEFAULT NULL
...
CREATE OR REPLACE FUNCTION public.register_patient_with_visit(
  ...
  p_consultation_service_id uuid DEFAULT NULL::uuid -- NEW PARAMETER
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_consultation_service_id IS NOT NULL THEN
    ...
    -- If invalid ID passed, we will fall back to auto-discovery or error?
    -- For now, if provided but not found, it stays NULL, so we might want to error out 
    -- if the user explicitly selected something that's now invalid. 
    -- But let's proceed to the fallback logic if it's null (safety net).
  END IF;
  ...
  -- Fallback 2: any consultation service for this facility
  IF v_consultation_service_id IS NULL THEN
    SELECT id, COALESCE(price, 0) INTO v_consultation_service_id, v_consultation_price
    FROM medical_services
    WHERE facility_id = p_facility_id
