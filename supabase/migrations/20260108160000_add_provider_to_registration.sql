-- Migration: Add provider parameter to register_patient_with_visit
-- Description: Allows specifying the requested provider (doctor/clinician) during registration.
...
CREATE OR REPLACE FUNCTION public.register_patient_with_visit(
  ...
  p_provider text DEFAULT 'Reception Intake'::text -- NEW PARAMETER with default for backward compatibility
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- UPDATED: Use p_provider instead of hardcoded 'Reception Intake'
  INSERT INTO visits (
